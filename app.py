    ...
    try:
        ...
        if doc_id in summaries_store:
            return jsonify({
                'success': True,
                'document_id': doc_id,
                'summary': summaries_store[doc_id]
            })
            
        # Generate summary
        doc_path = document_store[doc_id]['path']
        summary = summarizer.hybrid_summarize(doc_path)
        
        # Store summary
        summaries_store[doc_id] = summary
        
        # Extract key points
        key_points = summarizer.extract_key_points(doc_path)
        summaries_store[doc_id]['key_points'] = key_points
    ...
    try:
        ...
        if doc_id in extracted_figures_tables and 'figures' in extracted_figures_tables[doc_id]:
            ...
            for figure in figures:
                figure['url'] = f"/extracted/{doc_id}/figures/{os.path.basename(figure['path'])}"
...
def extract_tables(doc_id):
    ...
    try:
        ...
        if doc_id in extracted_figures_tables and 'tables' in extracted_figures_tables[doc_id]:
            ...
            # Update paths to be relative URLs
            for table in tables:
                table['url'] = f"/extracted/{doc_id}/tables/{os.path.basename(table['path'])}"
...
def analyze_citations():
    """Analyze citations across all documents"""
    try:
        # Process all documents for citation network
        for doc_id, doc in document_store.items():
            citation_network.extract_citations(doc['path'])
        
        # Generate visualization
        network_path = os.path.join(app.config['EXTRACT_FOLDER'], 'citation_network.png')
        vis_path = citation_network.visualize_network(output_path=network_path)

        # Return URL to the visualization
        return jsonify({
            'success': True,
            'visualization_url': f"/extracted/citation_network.png",
            'influential_papers': citation_network.get_influential_papers()
        })
    except Exception as e:
        return jsonify({'error': f'Error analyzing citations: {str(e)}'}), 500
...
@app.route('/extracted/<path:filename>', methods=['GET'])
def get_extracted_file(filename):
    """Serve extracted files (images, visualizations, etc.)"""
...
def add_to_zotero():
    ...
    # If a document_id is provided, use its metadata
    ...
    success = zotero.add_item(metadata, collection_key)
    if success:
        return jsonify({'success': True, 'message': 'Added to Zotero successfully'})
    else:
